<script lang="ts">
	...
	import { uploadFolder, scanFolder } from '$lib/apis/graden';
	...
	const uploadFolderHandler = async (folderPath: string) => {
		try {
			const result = await uploadFolder(localStorage.token, folderPath);
			if (result.uploaded > 0) {
				// Scan folder to get file list
				const scanResult = await scanFolder(localStorage.token, folderPath);
				if (scanResult.files) {
					// Create virtual files from the uploaded folder
					const folderFiles = scanResult.files.map((f: any) => ({
						id: `folder_${Date.now()}_${f.path}`,
						name: f.path,
						type: 'folder',
						size: f.size,
				...
				toast.success(`Dossier uploadé: ${result.uploaded} fichiers`);
			}
			if (result.errors > 0) {
				toast.warning(`${result.errors} erreurs lors de l'upload`);
			}
		} catch (error) {
			toast.error(`Erreur: ${error.message || 'Impossible d\'uploader le dossier'}`);
		}
	};
					...
					<div class="px-3 py-2">
						{#if window.pywebview}
							<p class="text-xs text-gray-500 mb-2">
								{$i18n.t('Cliquez pour ouvrir l\'explorateur de fichiers')}
							</p>
							<button
								class="w-full px-3 py-3 text-sm bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition flex items-center justify-center gap-2"
								on:click={() => {
									nativePickFolder();
								}}
							>
								<FolderOpen />
								{$i18n.t('Choisir un dossier')}
							</button>
						{:else}
							<p class="text-xs text-gray-500 mb-2">
								{$i18n.t('Entrez le chemin du dossier')}
							</p>
							<input
								type="text"
								id="folder-path-input"
								placeholder="C:\Users\grand\mon-projet"
								...
								on:keydown={(e) => {
									if (e.key === 'Enter') {
										...
										if (folderPath) {
											uploadFolderHandler(folderPath);
											show = false;
										}
									}
								}}
							/>
							<button
								class="mt-2 w-full px-3 py-2 text-sm bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition"
								on:click={() => {
