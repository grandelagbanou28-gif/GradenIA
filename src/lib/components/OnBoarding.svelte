	...
	function setLogoImage() {
		...
		if (logo) {
			...
			if (isDarkMode) {
				...
				darkImage.src = `${WEBUI_BASE_URL}/static/graden_blue.png`;

				darkImage.onload = () => {
					logo.src = `${WEBUI_BASE_URL}/static/graden_blue.png`;
					logo.style.filter = '';
				};
			...
			<div class="flex flex-col justify-end w-full items-center pb-10 text-center">
				<div class="text-5xl lg:text-7xl font-secondary">
					<Marquee
						duration={5000}
						words={[
							$i18n.t('Explorez le code'),
							$i18n.t('Decodez les mystères'),
							$i18n.t('Forgez de nouveaux chemins'),
							$i18n.t('Plongez dans la connaissance'),
							$i18n.t('Découvrez les merveilles'),
							$i18n.t('Éveillez la curiosité'),
							$i18n.t('Créez l\'innovation'),
							$i18n.t('Révélez les secrets'),
							$i18n.t('Pionnez les insights'),
							$i18n.t('Embarquez pour l\'aventure')
						]}
					/>

					<div class="mt-0.5">{$i18n.t('Grden IA - Intelligence Artificielle')}</div>
				</div>

				<div class="flex justify-center mt-8">
					<div class="flex flex-col justify-center items-center">
						<button
							aria-label={$i18n.t('Commencer')}
