...
async def _raise_tts_error(exc: Exception, r=None) -> None:
    ...
    detail = 'Graden IA: Server Connection Error'
    if r is not None:
        try:
            res = await r.json()
            if 'error' in res:
    ...
    try:
        ...
        if r is not None:
            try:
                ...
                detail = f'External: {e}'
        raise Exception(detail if detail else 'Graden IA: Server Connection Error')
...
async def _transcribe_azure(request, file_path, filename, file_dir, id):
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=detail if detail else 'Graden IA: Server Connection Error',
        )
